...
object MnemonicCode {
    ...
    private fun toBinary(x: Byte): List<Boolean>{
        tailrec fun loop(x: Int, acc: List<Boolean> = listOf()): List<Boolean> = if (x == 0) acc else loop(x / 2, listOf((x % 2) != 0) + acc)
    ...
    tailrec fun group(items: List<Boolean>, size:  Int, acc : List<List<Boolean>> = emptyList()) : List<List<Boolean>> {
        return when {
            items.isEmpty() -> acc
    ...
    fun validate(mnemonics: List<String>, wordlist: List<String> = englishWordlist) {
        ...
        tailrec fun toBits(index: Int, acc: List<Boolean> = listOf()): List<Boolean> = if (acc.size == 11) acc else toBits(index / 2,  listOf(index % 2 != 0) + acc)
        ...
        val data = group(databits, 8).map { fromBinary(it) }.map { it.toByte() }.toByteArray()   // .grouped(8).map(fromBinary).map(_.toByte).toSeq
    ...
    fun toSeed(mnemonics: List<String>, passphrase: String): ByteArray {
        ...
        val key = Pbkdf2.generate(password, salt, 2048, 64, Sha512())
        return key
    }
