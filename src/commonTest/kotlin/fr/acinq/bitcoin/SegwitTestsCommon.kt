...
class SegwitTestsCommon {
    ...
    fun `tx hash`() {
        val tx =
            Transaction.read("")
        ...
        val priv = PrivateKey(Hex.decode("619c335025c7f4012e556c2a58b2506e30b8511b53ade95ea316fd8c3286feb901"))
        val pub = priv.publicKey()
        val sig =
            Hex.decode("304402203609e17b84f6a7d30c80bfa610b5b4542f32a8a0d5447a12fb1366d7f01cc44a0220573a954c4518331561406f90300e8f3358f51928d43c212a8caed02de67eebee")
        assertTrue(Crypto.verifySignature(hash, Crypto.der2compact(sig), pub))

        val sigScript =
            Hex.decode("4830450221008b9d1dc26ba6a9cb62127b02742fa9d754cd3bebf337f7a55d114c8e5cdd30be022040529b194ba3f9281a99f2b1c0a19c0489bc22ede944ccf4ecbab4cc618ef3ed01")
    ...
    fun `tx verification`() {
        ...
        val runner = Script.Runner(Script.Context(tx, 1, Satoshi(600000000)), ScriptFlags.STANDARD_SCRIPT_VERIFY_FLAGS)
    ...
    fun `create p2wpkh tx`() {
        ...
        val tx2 = {
            val tmp = Transaction(
                ...
                txOut = listOf(TxOut(Satoshi(39000000), Script.pay2wpkh(pub1))),
                lockTime = 0
            )
    ...
    fun `create p2wsh tx`() {
        ...
        val tx2 = {
            ...
            val tmp = Transaction(
                version = 1,
                txIn = listOf(TxIn(OutPoint(tx1.hash, 0), sequence = 0xffffffffL)),
                txOut = listOf(TxOut(Satoshi(49000000), Script.pay2wsh(redeemScript))),
                lockTime = 0
            )
            val sig = Transaction.signInput(
                tmp,
        ...
        val tx3 = {
            val tmp: Transaction = Transaction(
                version = 1,
                txIn = listOf(TxIn(OutPoint(tx2.hash, 0), sequence = 0xffffffffL)),
                txOut = listOf(TxOut(Satoshi(48000000), Script.pay2wpkh(pub1))),
                lockTime = 0
            )
    ...
    fun `create p2pkh embedded in p2sh`() {
        ...
        val tx1 = {
            val tmp: Transaction = Transaction(
                ...
                txOut = listOf(TxOut(Satoshi(49000000), listOf(OP_0, OP_PUSHDATA(pub1.hash160())))),
                lockTime = 0
            )
            val pubKeyScript = Script.pay2pkh(pub1)
            val sig = Transaction.signInput(
