...
class Musig2TestsCommon {
    ...
    fun `generate secret nonce`() {
        ...
        tests.jsonObject["test_cases"]!!.jsonArray.forEach {
            ...
            // We cannot use the test vectors that provide an aggregated public key, because it's unclear how we would
            // build the corresponding key aggregation cache that must be passed as argument.
            if (aggpk == null) {
                val (secnonce, pubnonce) = SecretNonce.generate(randprime, sk, pk, msg?.byteVector32(), null, extraInput?.byteVector32())
