...
class TransactionTestsCommon {
    companion object {
        fun readData(filename: String): JsonElement {
            val file = FileSystem.currentDirectory.resolve(filename)
    ...
    fun `reference valid tx tests`() {
        ...
        val tests = readData("src/commonTest/resources/data/tx_valid.json")
    ...
    fun `reference invalid tx tests`() {
        val tests = readData("src/commonTest/resources/data/tx_invalid.json")
